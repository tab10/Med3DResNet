...
class ResNet(object):
    def __init__(self, sess, args):
        ...
        if self.dataset_name == 'ACV':
            self.train_x, self.train_y, self.train_z, self.test_x, self.test_y, self.test_z, n_channels = \
                load_ACV(path=args.path, flag=args.data_type, lungmask=args.lung_mask)
            self.img_size = 256  # 3D
            self.c_dim = n_channels
            self.label_dim = 4
    ...
    def build_model(self):
        """ Graph Input """
        self.train_inptus = tf.placeholder(tf.float32, [self.batch_size, self.img_size, self.img_size, self.img_size, self.c_dim], name='train_inputs')
        self.train_labels = tf.placeholder(tf.float32, [self.batch_size, self.label_dim], name='train_labels')

        self.test_inptus = tf.placeholder(tf.float32, [len(self.test_x), self.img_size, self.img_size, self.img_size, self.c_dim], name='test_inputs')
